...
let consolidado = [...radiologia, ...traumatologia,...dental];
let informacionTabla = document.querySelector('#informacionTabla');
...
document.write(`Primera atención: ${traumatologia[0].paciente} - ${radiologia[0].prevision}| Última atención: ${traumatologia[traumatologia.length-1].paciente} - ${traumatologia[traumatologia.length-1].prevision}`);

document.write('<h2>Dental</h2>')
document.write(`Primera atención: ${dental[0].paciente} - ${radiologia[0].prevision} | Última atención: ${dental[dental.length-1].paciente} - ${dental[dental.length-1].prevision}`);

/*3. Recorrer el arreglo y mostrar su contenido en una tabla*/

consolidado.forEach((element) =>{
    informacionTabla.innerHTML += `<tr>
    <th>${element.hora}</th>
    <th>${element.especialista}</th>
    <th>${element.paciente}</th>
    <th>${element.rut}</th>
    <th>${element.prevision}</th>
    </tr>`
})
